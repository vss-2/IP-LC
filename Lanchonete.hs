...
-- Sua função: verifica se após fim (modificado pelas threads) chegou a zero. Encerra o loop caso sim, encerrando o main que encerra as threads
...
main = do
    ...
    -- Controla a concorrência (quantidade de execuções e o objeto de disputa: faca)
    let qntExec = 20
    lockFaca <- newMVar 0
    ...
    -- Iniciando as threads: 1 produtora 2 fornecedoras
    forkIO(fornecedor pao carne tomate qtdExec)
    forkIO(produtor pao carne tomate lockFaca qtdExec)
    forkIO(produtor pao carne tomate lockFaca qtdExec)

    -- O main chamou as threads, elas executam waitThreads e depois o código acaba
